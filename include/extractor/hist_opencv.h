...
#ifndef FRAMEEXTRACTOR_HIST_OPENCV_H
#define FRAMEEXTRACTOR_HIST_OPENCV_H
...
namespace extractor{
    ...
    inline void calc_gray_hist(const Mat& img, Mat& hist, int nbins=128)
    {
        ...
        cv::calcHist(&img, 1, 0, Mat(), hist, 1, &nbins, &_range_gry, true, false);
    ...
    inline void calc_edge_hist( const Mat& Gx, const Mat& Gy, Mat& hist, int nbins_ori=16, int nbins_mag=16)
    {
        ...
        if(!hist.data)
            hist = Mat(nbins_mag + nbins_ori, 1, CV_32F, Scalar(0,0,0)[0]);
    ...
    inline void calc_pyr_edge_hist(const Mat& img, Mat& hist, int nbins_ori=16, int nbins_mag=16, int level=2)
    {
        ...
        int hist_sz = nbins_mag + nbins_ori;
...
#endif //FRAMEEXTRACTOR_HIST_OPENCV_H
